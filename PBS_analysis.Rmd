...
**Abbreviations**: PAG: phenylacetylglutamine, PCS: p-cresol sulfate, IS: indoxyl sulfate, PHPA: p-hydroxyphenylacetate, unkn: unknown, aKIV: alpha-ketoisovalerate, MM: methylmalonate, THB: D-beta-hydroxybutyrate, NAGP: N-acetyl glycoprotein  
...
input_data <- subset(integrals, integrals$Group != "Restrictive")
...
plot_colors <- c("purple", "darkturquoise", "coral2")
...
### Figures S2 and S4: Overlayed median 1H NMR spectra

Serum (Figure S2) and urine (Figure S4) median spectra were overlayed for the three study groups

```{r med_spec}

plot_med_specs <- function(to_plot, save_dir, save_name) {
  
  p <- plot_ly(to_plot, x = ~ppm, y = ~MAL, name = "MAL", type = "scatter", mode = "lines", alpha = 0.8, color = color_pal$MAL) %>%
...
# urine
plot_med_specs(med_specs_urine, save_dir, "figureS4_median_spectra_urine.html")

```

![Figure S2](output/figureS2_median_spectra_serum.png)
![Figure S4](output/figureS4_median_spectra_urine.png)

### Table S4: Maternal metabolic changes depend on type of bariatric surgery
...
tableS4_formatted <- tableS4 %>%
  mutate(P = formatC(tableS4$p.val, format = "e", digits = 2),
            Padj = formatC(tableS4$p.adj.BH, format = "e", digits = 2))

# change non-significant p values to NS
tableS4_formatted$P[as.numeric(tableS4_formatted$P) > 0.05] <- "NS"
tableS4_formatted$Padj[as.numeric(tableS4_formatted$Padj) > 0.05] <- "NS"

```

```{r save_tableS4, echo=FALSE}

write.csv(tableS4_formatted[,c("Metabolite","Time_point","Group","log2FC","P","Padj")], 
          file = file.path(save_dir, "tableS4_metabolic_changes.csv"), row.names = F)
...
### Figure S6: Relative concentrations of urinary host-microbial co-metabolites vary depending on bariatric surgery sub-type
...
figS6 <- ggarrange(PAG_T1_boxplot, PAG_T2_boxplot, PAG_T3_boxplot, PAG_T4_boxplot, 
...
```{r save_figureS6, echo=FALSE}

ggsave(figS6, filename = file.path(save_dir, "figureS6_metab_BS_subgroup.pdf"), height = 150, width = 170, units = "mm")
figS6
...
Generate differential abundance table
...
write.csv(MWU_filt, file.path(save_dir, "tableS5_changes_in_taxa_abund.csv"), row.names = F)
...
### Table S6: Differentially abundant taxa in malabsorptive group
...
keep <- which(row.names(taxtab_rdp) %in% row.names(sigtab))
...
write.csv(species_assign, file.path(save_dir, "tableS6_diff_abund_asv.csv"))
...
ggsave(circosPlot(m,
                  ...
                  color.blocks = c("darkolivegreen", "darkgoldenrod3", "rosybrown3"),
...
![Figure 3](output/figure3_integration.jpg){width=110%}
...
![Figure 4 (generated with Cytoscape)](output/figure4_correlation_net.jpg){width=110%}

### Table S7: Relative concentrations of metabolites are negatively associated with maternal insulin resistance and fetal/birth weight
...
write.csv(corr_metaVmetab_sig, file.path(save_dir, "tableS7_partial_corr.csv"), row.names = FALSE)

```

### Figures S7: Key metabolites are not associated with percentage of weight lost or length of time since surgery in malabsorptive patients
...
```{r save_figS7, echo=FALSE}

ggsave(p, 
       filename = file.path(save_dir, "figureS7_corr_PWL_TSS.pdf"),
       height = 5.5, width = 9)
p
